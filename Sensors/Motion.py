...
class Motion:
    ...
    def get_has_motion(self):
        ...
        if motionRead is not None:
            ...
            self.last_successful_read_time = curr_time
            self._remove_old_timestamps(curr_time)
            if motionRead == 1:
                self.motion_timestamps.append(curr_time)
        elif self.last_successful_read_time and curr_time - self.last_successful_read_time > datetime.timedelta(seconds=30):
            self.last_successful_read_time = None
        return ((len(self.motion_timestamps) > 4) and (len(self.motion_timestamps) < 10)) if self.last_successful_read_time else None

    def _remove_old_timestamps(self, curr_time):
        while len(self.motion_timestamps) > 0 and curr_time - self.motion_timestamps[0] > datetime.timedelta(seconds=10):
