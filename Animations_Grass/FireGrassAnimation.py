...
class FireGrassAnimation(GrassAnimation):
    ...
    def create_effects(self):
        ...
        for leaf in self.grass.get_leaves_array():
            ...
            self.effects.append(GradientEffect(down_flame, min_color, max_color, 4))

            blue_color = [100, 50, 0]

            up_fire = up[:2:]
            down_fire = down[-2::]
            self.effects.append(GradientEffect(up_fire[::-1], min_color, blue_color, 3))
            self.effects.append(GradientEffect(down_fire, min_color, blue_color, 3))
